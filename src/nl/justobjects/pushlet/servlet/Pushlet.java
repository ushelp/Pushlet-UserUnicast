...
public class Pushlet extends HttpServlet implements Protocol {
	...
	public static final Map<String, Vector<String>> UNICAST_USERS = new HashMap<String, Vector<String>>();
		...
		if (sessionId != null) {
			...
			if (userId != null) {
				...
				if (unicastType.equalsIgnoreCase("first")) {
					// first，仅推送给第一个连接的用户
					// 没有用户连接时，才加入推送，已有用户连接，后连接用户不推送
					if (UNICAST_USERS.get(userId).size() == 0) {
						// 将“用户名”和“pushlet的SessionID”关联起来
						UNICAST_USERS.get(userId).add(sessionId);
					}

				} else if (unicastType.equalsIgnoreCase("last")) {
					// last，推送给最后一个连接的用户
					// 仅推送给最后连接的用户
					UNICAST_USERS.get(userId).clear();
					UNICAST_USERS.get(userId).add(sessionId);
				} else {
					// all，推送给所有连接用户
					// 为用户添加新会话
